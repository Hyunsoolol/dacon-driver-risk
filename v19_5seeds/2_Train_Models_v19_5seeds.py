## ----------------------------------------------------------------
## [Jupyter용] 2_Train_Models_v19_5seeds.py
##  - v19 구조 유지 + 5-seeds 앙상블
...
print("Big-Tech ML Engineer (v19-5seeds): K-Fold + Leakage Fix + Domain Features 시작.")
...
def train_model_A(X_train, y_train, X_val, y_val, seed, group_label="A"):
    ...
    print(f"\n[{group_label}] CatBoost (Base+Hist+Norm+log_ratio, no PK, no Delta) "
...
def train_model_B(X_train, y_train, X_val, y_val, pk_stats_fold, seed, group_label="B"):
    ...
    print(f"[{group_label}] CatBoost (Base+PK+Hist+Norm+Delta+log_ratio) "
          f"학습 시작... (seed={seed}, 피처 {len(cb_X_train.columns)}개)")
    cat_base_model = cb.CatBoostClassifier(
        iterations=3000,
        loss_function='Logloss',
...
for fold in range(N_SPLITS):
    ...
    for seed in SEEDS:
        # --- 모델 A 학습 ---
        print(f"\n--- 모델 A (seed={seed}) (PK Stats 미사용, Delta 제외, log_ratio 포함) 학습 ---")
        cat_A, calib_A = train_model_A(
            X_train_A, y_train_A, X_val_A, y_val_A,
            seed=seed,
            group_label=f"A_fold{fold}_s{seed}"
        )
        ...
        # --- 모델 B 학습 ---
        print(f"\n--- 모델 B (seed={seed}) (PK Stats + Delta + log_ratio, tuned) 학습 ---")
        if len(X_train_B) > 0 and len(X_val_B) > 0 and len(np.unique(y_train_B)) > 1:
            cat_B, calib_B = train_model_B(
...
print("\n[INFO] '최종 보정' CatBoost v19-5seeds 모델 (A/B * 5seeds * 5fold) 및 최종 PK 통계 피처 저장 완료:")
print(f"  - PK Stats: {final_pk_stats_path}")
print("Big-Tech ML Engineer (v19-5seeds): 미션 완료.")
